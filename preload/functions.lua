...
function property(class, varName, defaultValue, createGetter, createSetter)
    ...
    local initValueIsTable = type(defaultValue) == "table"
    if createGetter then
        -- FIX 默认属性为table的时候 self[varName] 应该重新创建一个表
        class[string.format("get%s", propName)] = function(self)
            if initValueIsTable then
                self[varName] = {}
                initValueIsTable = false
            end
